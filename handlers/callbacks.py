...
async def process_show_user_agreement(callback_query: types.CallbackQuery):
    USER_AGREEMENT_TEXT = """
        ...
        Настоящее Соглашение регулирует отношения между владельцем бота (далее — «Администратор») и пользователем (далее — «Пользователь»).
        Нажимая кнопку "Start" Пользователь автоматически соглашается с приведенными ниже условиями.
        
        *1. Предмет соглашения*
        1.1. Администратор предоставляет Пользователю доступ к функционалу бота на условиях настоящего Соглашения.
        1.2. Используя бота, Пользователь подтверждает, что полностью принимает все условия данного Соглашения.

        *2. Сбор и хранение данных*
        2.1. Единственные данные, которые собирает и хранит бот — это Ваш Telegram ID.
        2.2. Администратор *не собирает и не хранит*: 
        • Имена и фамилии
        • Номера телефонов
        • Адреса электронной почты
        • Историю переписки
        • Любые другие персональные данные
        2.3. Telegram ID используется исключительно для идентификации пользователя в рамках функционала бота.

        *3. Использование данных*
        3.1. Telegram ID не передаётся третьим лицам.
        3.2. Telegram ID хранится на защищённом сервере.
        3.3. Telegram ID может быть удалён по запросу пользователя.

        *4. Права и обязанности*
        4.1. Пользователь обязуется не использовать бота в противоправных целях.
        4.2. Администратор оставляет за собой право изменять условия Соглашения с уведомлением пользователей.

        *5. Отказ от ответственности*
        5.1. Бот предоставляется «как есть», Администратор не гарантирует бесперебойную работу сервиса.
        5.2. Пользователь использует бота добровольно и на свой страх и риск.

        По вопросам удаления данных обращайтесь к Администратору.
    """
